...
<section class="module module--pacientes">
    ...
    <form method="POST" action="?url=pacientes" class="module-search" style="margin-bottom: 1rem; display: flex; gap: 8px; flex-wrap: wrap;">
        <input type="text" name="buscar" placeholder="Buscar por nombre, apellido o cédula..." value="<?= htmlspecialchars($busqueda); ?>" style="flex:1;min-width:220px;padding:0.5rem 0.75rem;border:1px solid #e2e8f0;border-radius:0.5rem;">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="insumos-table__actions">
                                    <a href="?url=pacientes&amp;editar=<?= (int) ($paciente['id_paciente'] ?? 0); ?>" class="insumos-table__action">Editar</a>
                                    <form method="POST" action="?url=pacientes" style="display:inline;" onsubmit="return confirm('¿Eliminar este paciente?');">
...
<div class="modal<?= $esEdicion ? ' is-open' : ''; ?>" id="modal-pacientes" aria-hidden="<?= $esEdicion ? 'false' : 'true'; ?>">
    ...
    <div class="modal__dialog" role="dialog">
        ...
        <form method="POST" action="?url=pacientes" class="modal__form">
