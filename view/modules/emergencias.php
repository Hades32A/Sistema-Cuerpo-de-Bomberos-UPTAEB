...
<section class="module module--emergencias">
    <header class="insumos-header">
        ...
        <button type="button" class="insumos-header__btn" data-modal-open="modal-emergencias">+ Registrar emergencia</button>
    </header>

    <form method="POST" action="?url=emergencias" class="module-search" style="margin-bottom: 1rem; display: flex; gap: 8px; flex-wrap: wrap;">
        <input type="text" name="buscar" placeholder="Buscar por tipo o ubicación..." value="<?= htmlspecialchars($busqueda); ?>" style="flex:1;min-width:220px;padding:0.5rem 0.75rem;border:1px solid #e2e8f0;border-radius:0.5rem;">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="insumos-table__actions">
                                    <a href="?url=emergencias&amp;editar=<?= (int) ($incidente['id_emergencia'] ?? 0); ?>" class="insumos-table__action">Editar</a>
                                    <form method="POST" action="?url=emergencias" style="display:inline;" onsubmit="return confirm('¿Eliminar esta emergencia?');">
...
<div class="modal<?= $esEdicion ? ' is-open' : ''; ?>" id="modal-emergencias" aria-hidden="<?= $esEdicion ? 'false' : 'true'; ?>">
    ...
    <div class="modal__dialog" role="dialog">
        ...
        <form method="POST" action="?url=emergencias" class="modal__form">
