...
<section class="module module--vehiculos">
    <header class="insumos-header">
        ...
        <button type="button" class="insumos-header__btn" data-modal-open="modal-vehiculos">+ Registrar vehículo</button>
    </header>

    <form method="POST" action="?url=vehiculos" class="module-search" style="margin-bottom: 1rem; display: flex; gap: 8px; flex-wrap: wrap;">
        <input type="text" name="buscar" placeholder="Buscar por placa, marca o modelo..." value="<?= htmlspecialchars($busqueda); ?>" style="flex:1;min-width:220px;padding:0.5rem 0.75rem;border:1px solid #e2e8f0;border-radius:0.5rem;">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="insumos-table__actions">
                                    <a href="?url=vehiculos&amp;editar=<?= urlencode((string) ($unidad['id_vehiculo'] ?? '')); ?>" class="insumos-table__action">Editar</a>
                                    <form method="POST" action="?url=vehiculos" style="display:inline;" onsubmit="return confirm('¿Eliminar este vehículo?');">
...
<div class="modal<?= $esEdicion ? ' is-open' : ''; ?>" id="modal-vehiculos" aria-hidden="<?= $esEdicion ? 'false' : 'true'; ?>">
    ...
    <div class="modal__dialog" role="dialog">
        ...
        <form method="POST" action="?url=vehiculos" class="modal__form">
